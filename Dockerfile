...
RUN apt-get update &&\
    ...
    sed -i /etc/pam.d/common-auth -e '/pam_unix.so/a auth    requisite                       pam_oidc.so oidc_config_url=http://localhost:4444/.well-known/openid-configuration client_id=test client_secret=some-secret redirect_uri=http://localhost:4444/callback scope=openid,offline'
